...
public static class InitialSet
{
    ...
    static readonly Regex DebugTypePattern = new Regex(@"(?<=<PropertyGroup.+?Configuration.+?==.+?Release.+?>.+?<DebugType>).+?(?=</DebugType>.+?</PropertyGroup>)", RegexOptions.Singleline);
    static readonly Regex RevisionPattern = new Regex(@"(?<!^\s*//.*)(?<=Assembly((File)|(Informational))Version\(""\d+\.\d+\.\d+)\.\d+", RegexOptions.Multiline);
